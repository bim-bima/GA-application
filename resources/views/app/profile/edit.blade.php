...
<div class="card shadow mb-4" data-aos="fade-up" data-aos-delay="50">
  ...
  <div class="card-body px-sm-3 px-2">
    <form action="{{ route('update_profile') }}" method="POST" enctype="multipart/form-data" class="col-lg-6 px-0">
      ...
      <input type="text" class="mb-1 form-control @error('name') is-invalid @enderror" name="name" required autofocus data-aos="fade-right" data-aos-delay="800" value="{{ $name }}" >
      @error('name')
      <div class="invalid-feedback">{{ $message }}</div>
      @enderror

      <label for="email" class="form-label" data-aos="fade-right" data-aos-delay="700"> email</label>
      <input type="email" class="mb-1 form-control @error('email') is-invalid @enderror" name="email" required autofocus data-aos="fade-right" data-aos-delay="800" value="{{ $email }}" >
      @error('email')
      <div class="invalid-feedback">{{ $message }}</div>
      @enderror
